...
//
    ...
    Workbook.prototype.substitute = async function(sheetName, substitutions) {
        ...
        sheetData.findall("row").forEach(async function(row) {
            ...
            row.findall("c").forEach(async function(cell) {
                var appendCell = true;
                ...
                if(cell.attrib.t === "s") {
                    ...
                    self.extractPlaceholders(string).forEach(async function(placeholder) {
    ...
    Workbook.prototype.substituteTable = async function(row, newTableRows, cells, cell, namedTables, substitution, key, placeholder, drawing) {
        ...
        if(substitution.length === 0) {
            ...
            substitution.forEach(async function(element, idx) {
                var newRow, newCell,
            ...
            if(typeof(imageObj) === 'string'  || imageObj instanceof String){
                ...
                if(this.isUrl(imageObj)){
                    ...
                    const buffer = await new Promise((res, rej) => {
                        request({url: imageObj, encoding: null}, (err, resp, buffer) => {
                            if(err) {
                                rej(err);
                            } else {
                                res(buffer);
